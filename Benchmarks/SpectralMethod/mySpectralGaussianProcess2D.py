...
class MySpectralGaussianProcess2D:
    def __init__(self,spectralModel, spatialMesh, discretization, upperbound):
        ...
        self.fmax = [] #désigne la fréquence maximale dans chaque arête spectrale
        ...
        self.nb_effective_vertices = self.discretization[0]*self.discretization[1]
    ...
    def __computeAlpha(self):
        ...
        for j in range(self.discretization[1]):
           for i in range(self.discretization[0]):
              self.alpha[pos] = factor * self.__computeSpecialExpo((i,j))
              pos += 1
            
       
    def __getFk(self, k):
        
        return [(0.5+k[0])*self.spectralSteps[0],(0.5 + k[1])*self.spectralSteps[1] - self.fmax[1] ]
    ...
    def __createCholeskyMatrices(self):
        ...
        for j in range(self.discretization[1]):
            for i in range(self.discretization[0]//2):
                 hermMatrix = self.spectralModel(self.__getFk((i,j)))
    ...
    def __computeZpk(self):
        ...
        for p in range(n):
           result.append(ot.ComplexMatrix(self.discretization[0],self.discretization[1]))
        ...
        for j in range(self.discretization[1]):
            for i in range(nHalves):
               ...
               for p in range(n):
                  (result[p])[nHalves + i,j] = resultRight[p] #ce choix de la conjugaison permet d'appliquer la FFT inverseTransform
                  (result[p])[nHalves-1-i,self.discretization[1]-1-j] = resultLeft[p].conjugate()
    ...
    def __computeXNp(self,zpk):
        ...
        for p in range(n):
           pseudoYNp = self.fft_algorithm.inverseTransform2D(zpk[p])
           pos = 0
           for j in range(self.discretization[1]):
             for i in range(self.discretization[0]):
                temp = self.alpha[pos]*pseudoYNp[i,j]
